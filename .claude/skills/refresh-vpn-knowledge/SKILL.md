...
- `MODE=check` — ответ уже в снимке выше, слой отбираю глазами по пути (`_live/`,
  `_reference/`, `_meta/`). Хода на замер не трачу, сразу докладываю оператору.
- `MODE=full` или домен не `vpn` — снимок не подходит (нужен JSON для дальнейших шагов
  либо другой домен), прогоняю скрипт ниже.
- Написано «НЕ СОБРАНО» — снимка нет, работаю только скриптом.

Снимок снят **один раз**, время съёмки указано в первой строке. Правил файлы знаний в этой
же сессии — цифры устарели, перезамеряй скриптом.
