...
## Грабли самой процедуры скана (не скрипта)

Эти четыре ловушки живут в шагах `SKILL.md`, а не в `dump-snapshot.sh`. Все —
ИСПРАВЛЕНЫ, здесь оставлены как объяснение, почему шаги написаны именно так.

**Ложный drift на все контейнеры** (находка /retro 2026-06-14). Симптом: Шаг 3 грепал
`container_name:` по `services.md`, а тот ведёт контейнеры таблицей `| имя | … |` → diff
показывал «20 недокументированных». Лечение: ось A — снимок-к-снимку по
`containers-inspect.json`; ось B — таблично-aware проверка имени в `services.md`.

**TLS-expiry не считался на acme.sh-хостах** (находка /retro 2026-06-14). Симптом:
`tls-certs.txt` содержал только `ls -la` (даты файлов), хотя `description` обещает «даты
валидности». Причина: openssl бежал только по `/etc/letsencrypt/live`. Лечение:
`openssl x509 -enddate` теперь и по `~/.acme.sh/*/fullchain.cer`.

**`HOST_DIR` из SSH-аргумента раздваивал inventory** (находка /retro 2026-06-14). Симптом:
алиас `<ssh-алиас>` создавал папку `prod-<алиас>` вместо записанной в конфиге
`prod-<ip>`. Лечение: канон из `infra-config.json` `servers[].alias` передаётся в скрипт
через env `HOST_DIR`; при расхождении с SSH-target скрипт громко предупреждает и берёт
канон.

**Verify заваливал валидный малый снимок** (находка /retro 2026-06-14). Симптом: порог
«суммарный размер ≥ 1 МБ» давал false-negative на валидном снимке 324 КБ. Лечение:
проверка непустоты **ключевых** файлов (набор зависит от `host_kind`) + парсинг
`containers-inspect.json` через jq — не суммарный размер.

**Хост без Docker не мог получить inventory вовсе** (2026-07-24, кейс ученика: нативный
VPS с 3X-UI + nginx). Симптом: `dump-snapshot.sh` делал fail-fast «Docker не найден →
exit 1», и владельцу нативного сервера оставалась только ручная сверка. Лечение
(ADR-0024): Docker — определяемое свойство (`host_kind`), секции контейнеров помечаются
`NOT_APPLICABLE`, добавлена секция `firewall.txt`, verify зависит от типа хоста.
Регрессия закрыта `tests/test-native-host.sh` (прогон вручную:
`bash .claude/skills/inventory-scan/tests/test-native-host.sh`).

---

## Дрейф IaC: почему чек Шага 3 написан именно так

**Откуда взялся чек.** Прежние проверки сравнивали **inventory** с реальностью. Но у
оператора есть третья сущность — **репозиторий конфигураций** (`$INFRA/services/`), из
которого конфигурация раскладывается по серверу. Между ним и сервером дрейф не ловил
никто, и он растёт в обе стороны.

**Что нашёл первый же разбор на живой инфраструктуре (2026-07-24):**

1. шесть vhost от снятых сервисов — включены на сервере, в репозитории их нет;
2. рабочий конфиг дашборда, которого **не было в репозитории вовсе** — восстановление из
   git дало бы неполный nginx;
3. compose снятого мониторинг-стека, оставшийся в списке деплоя — вызов, потерявший
   адресата.

**Отсюда правило «направление важнее факта».** «Лишнее в репозитории» — беспорядок,
терпит. «Есть на сервере, нет в репозитории» — **потеря при восстановлении**: этот класс
идёт оператору первым.

**Отсюда же правило «кандидат в мусор ≠ мусор».** В том же разборе каталог стека без
контейнера был в шаге от предложения снести — спасла проверка `systemctl`, показавшая
активный юнит: каталог обслуживал хост-сервис вне Docker. Прежде чем предлагать снос,
ищи упоминания имени в `/etc/systemd/system/`, `/etc/cron.d/` и host-скриптах; нашёл —
пиши «используется, не трогать», а не «кандидат в мусор».

---

## Граничные случаи скана

- **Сервер недоступен (down)** — скилл валит с явной ошибкой ещё на Pre-check, не
  генерирует пустой snapshot.
- **Disk full на сервере** — некоторые секции снимка собраны частично, отчёт явно
  говорит «частичный snapshot, причина: disk full». Частичным данным в drift-report
  не доверяем.
- **Контейнер в restart loop** — попадает в снимок со статусом `Restarting (N)`,
  в drift-report помечается отдельно как «требует внимания».
- **Несколько серверов** — переключаются параметром `SSH_HOST`. Одновременно не
  запускать: лок Шага 1 один на весь `inventory/`, второй скан упрётся в «уже идёт».
