...
## Четыре слепых пятна снимка (вскрыты сканом Bronto, исправлены 2026-07-10)

Все четыре нашлись за один боевой прогон на Bronto (первый скан после cutover):

1. **root-crontab выглядел пустым.** `crontab -l` без sudo читает crontab
   SSH-пользователя (vefmv), а не root — автопродление acme.sh «отсутствовало».
   Фикс: `sudo -n crontab -l` для root + отдельная секция для SSH-пользователя;
   sudo нет → честная пометка «нет данных», не «пусто».
2. **Скрипты IaC-раскладки невидимы.** Glob `/opt/*.sh` не видел
   `/opt/infra/scripts/**` — собственную раскладку агента (все скрипты в git-репо
   инфры). Фикс: `find /opt -maxdepth 4` с prune node_modules/venv/.git/.cache и
   потолком 100 файлов; содержимое глубоких скриптов в снимок не тянем — они в git.
3. **TLS-сертификаты в нестандартных путях не находились.** Поиск шёл только по
   `/etc/letsencrypt/live` и `~/.acme.sh` SSH-пользователя, а боевые сертификаты
   Bronto лежат в `/etc/nginx/ssl/` (кладёт acme.sh из-под root). Фикс: главный
   источник — фактические пути `ssl_certificate` из `nginx -T` (то, чем сервер
   реально отвечает), плюс `/root/.acme.sh` через `sudo -n`.
4. **Хост-сервисы вне Docker — полные невидимки.** Кейс newsforge: systemd-сервис
   с `User=` + venv не находился ни docker ps, ни compose-списком — из-за этого
   его БД посчитали бесхозной и удалили (восстановлена без потерь; см.
   `incidents/2026-07-10-drop-database-newsbot.md` инфры). Фикс: новый сбор
   `host-services.txt` — нештатные `/etc/systemd/system/*.service` с
   Description/User/ExecStart и состоянием; в `services.md` — отдельная секция
   «Хост-сервисы (не Docker)».
