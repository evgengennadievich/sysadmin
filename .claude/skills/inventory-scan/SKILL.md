...
**Чек «дрейф IaC»** (репозиторий инфраструктуры ↔ сервер) — вторая секция отчёта.

Прежние чеки сравнивают **inventory** с реальностью. Но у оператора есть третья сущность —
**репозиторий конфигураций** (`$INFRA/services/`), из которого конфигурация раскладывается
по серверу. Между ним и сервером дрейф не ловил никто, и он растёт в обе стороны. Разбор
2026-07-24 на живой инфраструктуре нашёл сразу: шесть vhost от снятых сервисов, рабочий
конфиг дашборда, которого **не было в репозитории вовсе** (восстановление из git дало бы
неполный nginx), и compose снятого мониторинг-стека, оставшийся в списке деплоя.
...
**Направление важнее факта.** «Лишнее в репозитории» — беспорядок. «Есть на сервере, нет в
репозитории» — **потеря при восстановлении**: об этом классе и говорить оператору первым.

**Кандидат в мусор ≠ мусор.** Каталог без контейнера может обслуживать хост-сервис: в том же
разборе `sysadmin-bot` был в шаге от сноса — спас проверка `systemctl`, показавшая активный
юнит. Прежде чем предлагать снос, ищи упоминания в юнитах, `cron.d` и скриптах; нашёл —
пиши «используется, не трогать».
