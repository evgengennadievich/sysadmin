...
# fallback, НО это риск раздвоить inventory: SSH-алиас даёт `prod-<алиас>`, а в
# документах хост мог быть записан как `prod-<ip>` — те же факты легли бы в две разные
# папки (находка /retro 2026-06-14). Поэтому при расхождении канона и выведенного —
# громко предупреждаем и берём канон. Конкретные алиас и IP здесь не приводим: C.4.
