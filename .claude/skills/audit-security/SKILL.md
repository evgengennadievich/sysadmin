...
**Вердикт по каждой проверке ставлю не на глаз, а по рубрике** — таблица «PASS / WARN /
FAIL» в конце `references/checklist.md`: там для каждой строки написан порог. Открываю её
до того, как писать сводку. Проверка, которую нечем подтвердить (нет доступа, пустой вывод),
идёт в отчёт как `НЕТ ДАННЫХ` с причиной и **не считается пройденной** (C.2).
