...
if [ "$SCOPE" = "all" ] || [ "$SCOPE" = "tls" ]; then
    ...
    if [ -n "$DOMAINS_FILE" ] && [ -f "$DOMAINS_FILE" ]; then
        ...
        # Источник проверки выбирается ОДИН раз и по явному признаку — есть ли
        # openssl на сервере, — а не по факту неудачи очередного домена.
        #
        # Почему так. Прежде логика была «пробуем на сервере, не вышло — уходим
        # на машину оператора». В «не вышло» попадал ЗАКОННЫЙ ответ: openssl
        # завершается ненулевым кодом, когда на порту нет сертификата, а это
        # штатная картина для VPN-инбаунда на 443. Проверка отрабатывала на
        # сервере, а отчёт писал «проверено с: рабочая машина оператора» — то
        # есть врал об источнике данных. Найдено при ручной верификации аудита
        # 2026-07-30: в отчёте стоял локальный источник при живом /usr/bin/openssl
        # на сервере.
        #
        # Врать об источнике здесь дороже, чем кажется: на Маке с включённым
        # TUN-клиентом VPN результат описывает путь через туннель, а не состояние
        # сервера. Оператор, читающий «проверено с сервера», принимает решение по
        # чужим данным.
        probe 0 "openssl version"
        if [ "$PROBE_OK" = "yes" ]; then
            ...
            add_recommendation "[ПРОВЕРЬ] openssl на сервере не найден — TLS проверялся с рабочей машины оператора. Результат описывает путь до домена ОТТУДА и при включённом VPN-клиенте может отличаться от того, что видит сервер. Поставь openssl на сервер, чтобы проверка была честной"
        else
            TLS_SOURCE="none"
            TLS_FROM="проверить нечем"
        fi
        ...
        for domain in $DOMAINS; do
            ...
            if [ "$TLS_SOURCE" = "none" ]; then
                add_result tls UNKNOWN "$domain" "openssl недоступен ни на сервере, ни на рабочей машине — проверить нечем"
                continue
            fi

            if [ "$TLS_SOURCE" = "server" ]; then
                # RC_MAX=255, а НЕ 0. Для этой команды ответом является любой код
                # возврата: судим по тому, пришёл ли сертификат в выводе. openssl
                # s_client выходит ненулевым и когда порт занят не-TLS сервисом, и
                # когда сертификат самоподписанный — оба случая суть ответ, а не
                # сбой выполнения. Маркер при RC_MAX=255 печатается тогда и только
                # тогда, когда оболочка на сервере действительно отработала.
                probe 255 "echo | openssl s_client -connect $domain:443 -servername $domain 2>/dev/null"
                if [ "$PROBE_OK" != "yes" ]; then
                    add_result tls UNKNOWN "$domain" "команда на сервере не выполнилась — проверка не состоялась (проверялось с: $TLS_FROM)"
                    continue
                fi
                CERT_RAW="$PROBE_OUT"
            else
