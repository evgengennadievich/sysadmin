...
# ═══════════════════════════════════════════════════════════════════════════
echo "[11] Страховка проверяется по поведению, а не по хвосту строки"
# ═══════════════════════════════════════════════════════════════════════════
# Проверщик 27.07.2026 живым запуском показал формы, где строка КОНЧАЕТСЯ на «|| true»,
# а код возврата всё равно ненулевой. Две самые дешёвые закрыты; остальные (set -e,
# trap … EXIT, pipefail) регуляркой не различить — это записано в ADR-0029 как предел
# грубого замка, а не как покрытие.
for spec in \
    'страховка спрятана в комментарии|!`false # || true`|ловить' \
    'явный ненулевой выход|!`exit 3 || true`|ловить' \
    'комментарий ПОСЛЕ страховки|!`echo ok || true # пояснение`|пропускать' \
    'обычная страховка|!`echo ok || true`|пропускать'
do
...
f="$(fixture_plain "dup.md" '`код` и !`false`')"
n="$(bash "$LINTER" --root "$WORK" "$f" 2>&1 | grep -c '^❌' || true)"
if [ "$n" -eq 1 ]; then ok "апостроф в прозе не удваивает находку (ровно 1)"
else bad "находок $n вместо 1 — обрезка режет не те апострофы"; fi

# Вторую подстановку клиент НЕ исполняет: перед `!` стоит апостроф, а не пробел.
f="$(fixture_plain "adjacent.md" '!`echo a || true`!`false`')"
if bash "$LINTER" --root "$WORK" "$f" >/dev/null 2>&1; then
    ok "подстановка вплотную к предыдущей не считается (клиент её не исполняет)"
else
    bad "ложная тревога на форме, которую клиент не исполняет"
fi

# heredoc внутри исполняемого блока обязан глушиться так же, как внутри bash-блока.
{ echo '```!'; echo "cat > /tmp/x.sh <<'SCRIPT'"; echo 'for f in $DIRS; do echo $f; done'; echo 'SCRIPT'; echo 'echo готово || true'; echo '```'; } > "$WORK/hd-exec.md"
if bash "$LINTER" --root "$WORK" "$WORK/hd-exec.md" >/dev/null 2>&1; then
    ok 'тело heredoc внутри исполняемого блока не проверяется (как и в bash-блоке)'
else
    bad 'heredoc внутри исполняемого блока даёт ложную тревогу'
fi

# Линейность: большой файл не должен занимать десятки секунд (была квадратика).
python3 -c "
import sys
with open(sys.argv[1],'w') as fh:
    for i in range(6000): fh.write('обычная строка %d\n' % i)
    fh.write('\`\`\`!\necho hi\n\`\`\`\n')" "$WORK/big.md"
t0=$(date +%s)
bash "$LINTER" --root "$WORK" "$WORK/big.md" >/dev/null 2>&1
t1=$(( $(date +%s) - t0 ))
if [ "$t1" -le 5 ]; then ok "6000 строк за ${t1}с — разбор линейный"
else bad "6000 строк за ${t1}с — похоже на квадратичный разбор"; fi
echo
