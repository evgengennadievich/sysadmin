...
# Строка ниже — та самая, дословно, какой она была в момент отказа.
BURN='!`r=$(git rev-parse --show-toplevel 2>/dev/null); s="$r/.claude/skills/_lib/check-knowledge-freshness.sh"; if [ -f "$s" ]; then echo "[снято $(date "+%Y-%m-%d %H:%M")]"; bash "$s" vpn --stale-only 2>&1; else echo "НЕ СОБРАНО"; fi`'

f="$(fixture_plain "subst-burn.md" "$BURN")"
if bash "$LINTER" --root "$WORK" "$f" >/dev/null 2>&1; then
    bad "НЕ поймал подстановку без страховки — ту самую, что убила ответ 27.07.2026"
else
    ok "поймал подстановку без страховки кода возврата"
fi

f="$(fixture_plain "subst-ok.md" "${BURN%\`} || true\`")"
if bash "$LINTER" --root "$WORK" "$f" >/dev/null 2>&1; then
    ok "пропустил ту же подстановку со страховкой (ложных срабатываний нет)"
else
    bad "ложное срабатывание: подстановка со страховкой должна проходить"
fi

# Подстановка идёт в оболочку оператора — значит и ОСТАЛЬНЫЕ 15 правил обязаны
# её проверять, а не только правило 16. Строка со страховкой, но с дроблением слов.
f="$(fixture_plain "subst-wordsplit.md" '!`for f in $KEY_FILES; do echo "$f"; done || true`')"
if bash "$LINTER" --root "$WORK" "$f" >/dev/null 2>&1; then
    bad "общие правила НЕ применяются к строке подстановки (дробление слов пропущено)"
else
    ok "общие правила применяются и к подстановке: дробление слов поймано"
fi
echo
